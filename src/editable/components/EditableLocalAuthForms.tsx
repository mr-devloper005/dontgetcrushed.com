...
const inputClass = 'h-12 w-full rounded-xl border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] px-4 text-sm font-medium text-[var(--slot4-page-text)] outline-none transition placeholder:text-[var(--slot4-soft-muted-text)] focus:border-[var(--slot4-accent)]'
const buttonClass = 'inline-flex h-12 w-full items-center justify-center rounded-full bg-[var(--slot4-dark-bg)] px-6 text-sm font-bold text-white transition hover:-translate-y-0.5 active:translate-y-0 disabled:cursor-not-allowed disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-xl px-4 py-3 text-sm font-semibold ${status === 'success' ? 'border border-[var(--slot4-accent)]/30 bg-[var(--slot4-accent-soft)] text-[var(--slot4-page-text)]' : 'bg-red-50 text-red-600'}`}>{message}</p> : null}
